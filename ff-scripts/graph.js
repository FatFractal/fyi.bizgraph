...
function unPrefix(s) {
    return s.replace("/ff/resources", "");
}

function _getGraph(queryResult, refs, gbItems) {
...
exports.getGraph = function() {
    ...
    // Get query, depthGb and depthRef from parameters
    // Execute the query
    // Execute the depthQuery
    // Process the results into something understandable by the graphing kit
    var params = ff.getExtensionRequestData().httpParameters;
    var query = params['query'];
    var depthRef = params['depthRef'];
    var depthGb = params['depthGb'];

    if (!query || (!depthRef && !depthGb))
        throw {statusCode:400, statusMessage:'Must supply query and either depthRef or depthGb'};
    if (!depthRef)
        depthRef=0;
    if (!depthGb)
        depthGb=0;

    var sort = params['sort'] || 'createdAt desc';
    var start = params['start'] || 0;
    var count = params['count'] || 2;
    
    var asUserGuid = ff.getActiveUser().guid;
    var base = ff.getArrayFromUri(query + "?sort=" + sort + "&start=" + start + "&count=" + count, asUserGuid);

    var depthResponse = ff.executeDepthQuery(base, depthRef, depthGb, asUserGuid);

    ff.response().result = _getGraph(base, depthResponse.references, depthResponse.grabBagItems);
